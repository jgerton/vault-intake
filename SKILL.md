...
| 3. Classify (mode-dependent) | Implemented (both modes; emergent uses frequency-based scoring against theme registry from top-level dirs + frontmatter) |
| 4. PARA category | Implemented (fixed_domains/para only; emergent skips PARA entirely via orchestrator guard) |
| 5. Generate frontmatter | Implemented (both modes; mode-conditional YAML emission with `theme` for emergent and `domain`+`project` for fixed_domains) |
| 6. Generate wikilinks | Implemented (both modes; emergent uses same-theme weight-4 signal in place of cross-domain) |
...
| Orchestrator: `run_intake` (dry-run) | Implemented (both modes end-to-end; v0.2.0 closes the emergent cascade — Steps 3, 5, 6 succeed natively, Step 4 is skipped by mode guard) |
...
- `--inbox` (M2: batch-process all `.md` files in `{vault}/inbox/`; mutually exclusive with `--input`; archives processed source files to `.vault-intake/inbox-processed/`)
...
| Vault structure (planned) | `_inbox/`, `_sinteses/`, plus emergent folders | `Areas/<domain>/sessions/`, `insights/`, `workflows/`, `prompts/`, `context/`, `projects/`, `references/` |
