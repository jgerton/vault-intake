...
| Orchestrator: `confirm_and_write` (post-confirmation file write) | Implemented (atomic write via temp file plus os.replace; live Step 9 invocation with frontmatter source_id mutation and atomic re-write; section-update path for context+project routes; defense-in-depth check destination is inside vault_path; CLI wrapper not yet) |

The orchestrator wires Steps 0-9 into two entrypoints that together return the spec's output contract per build spec lines 228-243. `run_intake(input_text, config, ...) -> IntakeRun` is the dry-run pass: `IntakeRun.written_path` is always None, and Step 9 always runs with `note_path=None` (returns a `skipped("dry-run...")` result). `confirm_and_write(intake_run, config, ...) -> IntakeRun` is the post-confirmation pass that writes the file, re-invokes Step 9 live, and re-writes the file with the updated YAML when Step 9 returns a `source_id`. The CLI wrapper that prompts the user between the two entrypoints lands in a follow-up commit; library helpers remain individually safe to use against a real vault.
