...
def test_session_project_routes_to_sessions_with_link_target(tmp_path):
    ...
    assert result.destination == tmp_path / "Areas" / "ops" / "sessions"
...
def test_route_does_not_create_destination_folder(tmp_path):
    ...
    assert not (tmp_path / "Areas" / "ops" / "sessions").exists()
...
def test_different_domain_routes_to_its_own_subfolder(tmp_path):
    ...
    assert result.destination == tmp_path / "Areas" / "branding" / "sessions"
